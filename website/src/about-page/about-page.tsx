    ...
    <section className="relative overflow-hidden bg-white px-6 py-32">
      ...
      <div className="relative mx-auto max-w-[75%]">
        ...
        <div className="mb-24 text-center">
          <h2 className="h1 leading-[0.85] text-zinc-900 ">
        ...
        <div className="absolute bottom-0 left-1/2 top-[250px] hidden w-[3px] -translate-x-1/2 bg-zinc-100 md:block" />
        <div className="absolute bottom-0 left-1/2 top-[250px] hidden w-[3px] -translate-x-1/2 bg-gradient-to-b from-[#3bc878] to-emerald-100 md:block" />

        <div className="space-y-24">
          {timelineData.map((item, idx) => (
            <div
              key={idx}
              className={`flex w-full flex-col items-center justify-between md:flex-row ${item.side === 'right' ? 'md:flex-row-reverse' : ''}`}
            >
              ...
              >
                <div className="relative rounded-[40px] border-2 border-[#3bc878] bg-white p-8 shadow-2xl shadow-emerald-100/50">
                  {/* The Pointing Triangle */}
                  <div
                    className={`absolute top-1/2 hidden h-4 w-4 -translate-y-1/2 rotate-[-45deg] border-b-2 border-r-2 border-[#3bc878] bg-white md:block ${item.side === 'left' ? '-right-2.5' : '-left-2.5 rotate-[135deg]'}`}
                  />

                  <span className="text-[16px]  font-bold  uppercase tracking-widest text-[#3bc878]">
                    {item.year}
                  </span>
                  <h2 className="h2  mb-4 mt-1 text-zinc-900 ">{item.title}</h2>
                  <p className="text-[17px] leading-[2em]  text-black">
                    {item.desc}
                  </p>
                </div>
              </motion.div>
              ...
              <motion.div
                ...
                className="mt-8 w-full md:mt-0 md:w-[38%]"
              >
                <div className="group relative h-[600px] overflow-hidden rounded-[40px] border-4 border-[#3bc878]/10">
                  <Image
                    src={item.img}
                    alt={item.title}
                    fill
