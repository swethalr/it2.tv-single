  ...
  return (
    ...
    >
      <div className="pointer-events-none relative flex h-1 w-1 flex-col items-center">
        ...
        >
          <h6 className="mb-0.5 whitespace-nowrap text-[8px] font-bold uppercase text-black lg:text-[10px]">
            {country}
          </h6>
...
// ✅ Accept onHoverChange callback
const GlobeModel = ({ onHoverChange }: { onHoverChange: (v: boolean) => void }) => {
  ...
  return (
    <group>
      ...
      {/* ✅ Only rotate when user is actually hovering the globe */}
      <OrbitControls
        enableZoom={false}
        enablePan={false}
    ...
    <div className={cn('relative flex h-full w-full items-center justify-center', className)}>
      <div className="relative h-full w-full" style={{ touchAction: 'none' }}>
        <Canvas
          ...
          dpr={[1, 2]}
          // ✅ Canvas only intercepts pointer events when globe is hovered
          style={{ pointerEvents: globeHovered ? 'auto' : 'none' }}
          gl={{ antialias: true }}
          className="!absolute !inset-[-25%] !h-[150%] !w-[150%] lg:!inset-0 lg:!h-full lg:!w-full"
        >
          <Suspense fallback={null}>
            <GlobeModel onHoverChange={setGlobeHovered} />
          </Suspense>
        </Canvas>
      </div>
    </div>
