...
export default function Technologies({ data }: { data: any }) {
  ...
  return (
    <section className="relative w-full overflow-hidden bg-white px-4 py-24">
      ...
      <div className="relative mx-auto hidden aspect-[12/7] w-full max-w-[1400px] sm:block">
        ...
        {/* Central Label 
        <div
          className="h4 absolute z-30 select-none rounded-2xl border-4 border-white/90 bg-black !important
          px-5 py-3 font-bold text-white shadow-[0_20px_50px_rgba(0,0,0,0.45)]"
          style={{
  ...
  style={{
    ...
    top: `${(centerY / 700) * 100}%`,
    transform: 'translate(-50%, -50%)',
    backgroundColor: '#000000', // Hardcoded fallback
      ...
      <div className="mt-8 grid grid-cols-2 gap-4 sm:hidden">
        {techItems.map((tech: any, i: number) => (
          <motion.div
            ...
            viewport={{ once: true }}
            className="flex flex-col items-center justify-center rounded-2xl border-2 bg-white  py-4  shadow-sm"
            style={{ borderColor: `${tech.color}25` }}
          >
            <div className="mb-2 text-3xl" style={{ color: tech.color }}>
              {tech.icon}
            </div>
          </motion.div>
        ))}
