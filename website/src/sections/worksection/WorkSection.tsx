          ...
          >
            {results.map((item, index) => (
              <SwiperSlide key={index} className="transition-all duration-500">
                {({ isActive }) => (
                  <div
                    className={`group relative h-[350px] max-w-[450px] overflow-hidden rounded-[30px]  bg-[#3cb878] transition-all duration-700 md:h-[500px] md:rounded-[40px] ${!isActive ? 'scale-90 opacity-100 ' : 'scale-100 opacity-100 shadow-2xl shadow-[#3cb878]/20'}`}
                  >
                    {/* Image */}
                    <Image
                      src={item.img}
