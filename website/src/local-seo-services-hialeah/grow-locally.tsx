'use client';
/**
 * src/local-seo-services-hialeah/sections/GrowLocally.tsx
 *
 * Section 1: Grow Your Business Locally
 * H2 heading + image + body paragraph + "Book a SEO Consultation" CTA → contact modal
 */

import React, { useState } from 'react';
import { motion } from 'framer-motion';
import { growLocallySection } from '../../data/local-seo-services-hialeah';
import { ContactModal } from './shared';

export function GrowLocally() {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const d = growLocallySection;
    ...
    >
      ...
      <div className="relative grid grid-cols-1 gap-12 lg:grid-cols-1">
        ...
        >
          ...
          <p className="p leading-[2em]">{d.body}</p>
        </motion.div>
      </div>
